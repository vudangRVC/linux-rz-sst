...
static void rsci_spi_set_rate(struct rsci_data *rsci)
{
	 ...
	 */
	if (!spi_controller_is_target(rsci->ctlr)) {
...
static void rsci_spi_remove(struct platform_device *pdev)
{
...
static int rsci_spi_probe(struct platform_device *pdev)
{
	...
	if (ret == RSCI_SPI_MASTER) {
		ctlr = spi_alloc_host(&pdev->dev, sizeof(struct rsci_data));
		slave = 0;
	} else {
		ctlr = spi_alloc_target(&pdev->dev, sizeof(struct rsci_data));
		slave = 1;
	}
	if (ctlr == NULL)
		return -ENOMEM;
