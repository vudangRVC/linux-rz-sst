 ...
 *  Copyright (C) 2024 Renesas Electronics Ltd
 */
...
static int PCIE_CFG_Initialize(struct rzv2h_pcie *pcie)
{
	...
	// Base Address Register Mask00 (upper) (Function #1) : PCIe Configuration Registers 60A4h
	...
	while (!kthread_should_stop()) {
		if (pcie_thread_status == RZV2_PCIE_THREAD_RESET) {
			...
			if ((reg & LTSSM_ST_ALL_MASK) == LTSSM_ST_DETECT) {
				...
				if (((reg & STATE_RECEIVER_DETECTED) >> 8)
						== pcie_receiver_detection) {
					tmp_cnt++;
					if (tmp_cnt >= 3) {
						...
						while (timeout--) {
							if (!(rzv2h_pci_read_reg(tmp_pcie,
									PCIE_CORE_STATUS_1_REG)
									& DL_DOWN_STATUS))
								break;
						...
						if (timeout) {
							reg = rzv2h_pci_read_reg(tmp_pcie,
									PCIE_CORE_STATUS_2_REG);
...
static void rzv2h_pcie_hw_enable_msi(struct rzv2h_pcie_host *host)
{
	...
	for (idx = 0; idx < MAX_NR_INBOUND_MAPS; idx++) {
		...
		pci_base |= (((unsigned long) rzv2h_pci_read_reg(pcie,
						AXI_DESTINATIONU_REG(idx))) << 32);
		msi_base_mask = rzv2h_pci_read_reg(pcie, AXI_WINDOW_MASKL_REG(idx));
		msi_base_mask |= (((unsigned long) rzv2h_pci_read_reg(pcie,
						AXI_WINDOW_MASKU_REG(idx))) << 32);
		if ((pci_base <= base) && (pci_base + msi_base_mask >= base)) {
			...
			msi_base |= (((unsigned long) rzv2h_pci_read_reg(pcie,
							AXI_WINDOW_BASEU_REG(idx))) << 32);
