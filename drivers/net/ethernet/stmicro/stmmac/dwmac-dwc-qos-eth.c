...
static void renesas_rzv2h_eqos_fix_speed(void *priv, int speed, unsigned int mode)
{
...
static int renesas_rzv2h_eqos_init(struct platform_device *pdev, void *priv)
{
	...
	writel(value, eqos->regs + GMAC4_MAC_ONEUS_TIC_COUNTER);
					...
					struct stmmac_resources *res)
{
	...
	if (of_device_is_compatible(np, "renesas,rzv2h-eqos"))
		data->flags |= STMMAC_FLAG_RX_CLK_RUNS_IN_LPI;
...
static void renesas_rzv2h_eqos_remove(struct platform_device *pdev)
{
