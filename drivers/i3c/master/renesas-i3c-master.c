...
static int renesas_i3c_master_bus_init(struct i3c_master_controller *m)
{
	...
	scl_rise_ns =  bus->scl_rate.i2c <= I2C_MAX_STANDARD_MODE_FREQ ? 500 :
			bus->scl_rate.i2c <= I2C_MAX_FAST_MODE_FREQ ? 100 : 75;
	scl_fall_ns = bus->scl_rate.i2c <= I2C_MAX_FAST_MODE_FREQ ? 100 : 75;
...
static void renesas_i3c_master_bus_cleanup(struct i3c_master_controller *m)
{
	...
	i3c_reg_update_bit(master->regs, RSTCTL, RSTCTL_RI3CTST,
							RSTCTL_RI3CTST);
	/* Wait for reset completion  */
	readl_relaxed_poll_timeout(master->regs + RSTCTL, val,
				!(val & RSTCTL_RI3CTST), 0, 1000);
}
...
static int renesas_i3c_master_probe(struct platform_device *pdev)
{
	...
	ret = clk_prepare_enable(master->pclkrw);
	if (ret)
		return ret;
...
static void renesas_i3c_master_remove(struct platform_device *pdev)
{
...
MODULE_DEVICE_TABLE(of, renesas_i3c_master_of_ids);
