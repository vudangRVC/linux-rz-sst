				  ...
				  const struct drm_display_mode *mode)
{
	 ...
	 */
	u32	hsfreq_high_limit = 445500;
	u32	hsfreq_mid_limit = 250000;

	if (dsi->info->type == MIPI_DSI_DPHY_RZV2H) {
		/*
		 * For RZV2H, hsfreq is in Hz, so multiply by 1000.
		 */
		hsfreq_high_limit = 445500 * 1000;
		hsfreq_mid_limit = 250000 * 1000;
	}

	if (hsfreq > hsfreq_high_limit) {
		clkkpt = 12;
		clkbfht = 15;
		clkstpt = 48;
		golpbkt = 75;
	} else if (hsfreq > hsfreq_mid_limit) {
		clkkpt = 7;
		clkbfht = 8;
		clkstpt = 27;
		golpbkt = 40;
...
static int rzg2l_mipi_dsi_attach(struct drm_bridge *bridge, struct drm_encoder *encoder,
					 ...
					 struct drm_atomic_state *state)
{
	...
	if (dsi->panel) {
		...
		/*
		 * Enable clock first to ensure LP/HS channel is ready
		 * before sending DCS commands. This avoids uncertainty
...
static int rzg2l_mipi_dsi_probe(struct platform_device *pdev)
{
	 ...
	 */
	unsigned long init_clock = 80000;
	if (dsi->info->type == MIPI_DSI_DPHY_RZV2H) {
		init_clock = 80000000;
	}

	ret = dsi->info->dphy_init(dsi, init_clock);
	if (ret < 0)
		goto err_phy;
